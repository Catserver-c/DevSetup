...
def install_vscode(e):
    file_path = os.path.join(download_folder, "VSCode.exe")
    urlretrieve("https://code.visualstudio.com/sha/download?build=stable&os=win32-x64",file_path)
...
def install_notepadpp(e):
    file_path = os.path.join(download_folder, "Notepadpp.exe")
    urlretrieve("https://github.com/notepad-plus-plus/notepad-plus-plus/releases/download/v8.8.6/npp.8.8.6.Installer.x64.exe",file_path)
...
def main(page: ft.Page):
    ...
    def show_languages(e):
        ...
        content_area.controls.extend([
            ft.ElevatedButton("Download Python", on_click=install_python),
            ft.ElevatedButton("Download C++", on_click=install_cpp),
            ft.ElevatedButton("Download JavaScript", on_click=install_js),
            ft.ElevatedButton("Download Java", on_click=install_java),
            ft.ElevatedButton("Download C#", on_click=install_cs),
            ft.ElevatedButton("Download Go", on_click=install_go),
            ft.ElevatedButton("Download Rust", on_click=install_rust),
            ft.ElevatedButton("Download PHP", on_click=install_php),
            ft.ElevatedButton("Download Ruby", on_click=install_ruby),
            ft.ElevatedButton("Download Swift", on_click=install_swift),
            ft.ElevatedButton("Download SQL", on_click=install_sql),
            ft.ElevatedButton("Download Perl", on_click=install_perl),
            ft.ElevatedButton("Download Lua", on_click=install_lua),
    ...
    def show_ides(e):
        content_area.controls.clear()
        content_area.controls.extend([
            ft.ElevatedButton("Download Android Studio", on_click=install_android_studio),
            ft.ElevatedButton("Download VS Code", on_click=install_vscode),
            ft.ElevatedButton("Download Postman", on_click=install_postman),
            ft.ElevatedButton("Download JetBrains Toolbox", on_click=install_jetbrains_toolbox),
        ])
        page.update()
    ...
    def show_editors(e):
        content_area.controls.clear()
        content_area.controls.extend([
            ft.ElevatedButton("Download Notepad++", on_click=install_notepadpp),
            ft.ElevatedButton("Download Sublime Text", on_click=install_sublime_text),
            ft.ElevatedButton("Download Atom", on_click=install_atom),
        ])
        page.update()
    
    # Кнопки навигации
    nav_buttons = ft.Row([
        ft.ElevatedButton("Programing languages", on_click=show_languages , color="green"),
        ft.ElevatedButton("IDE", on_click=show_ides, color="green" ),
        ft.ElevatedButton("Code editors", on_click=show_editors, color="green"),
    ])
    ...
    page.add(
        ft.Column([
            ...
            nav_buttons,
            content_area,
            ft.Text("Files will appear in SR_Downloads folder", color="gray")
        ])
    )
